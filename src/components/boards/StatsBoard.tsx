    ...
    <div className="flex flex-col lg:flex-row gap-6 lg:gap-10 items-start h-full animate-in fade-in slide-in-from-right-4 duration-500 overflow-visible relative pt-4 md:pt-6">
      ...
      {isAndroid ? (
        <>
          ...
          {/* Sidebar Drawer */}
          <div className={`fixed top-0 left-0 bottom-0 w-[280px] z-50 transform transition-all duration-300 ease-in-out border-r ${
            darkMode ? 'bg-black/95 backdrop-blur-3xl border-white/10 shadow-[20px_0_60px_-15px_rgba(0,0,0,0.8)]' : 'bg-white border-emerald-50'
          } ${isCalendarCollapsed ? '-translate-x-full shadow-none' : 'translate-x-0 shadow-2xl'}`}>
            <div className="h-full overflow-y-auto p-4 scrollbar-none">
              <div className="flex justify-between items-center mb-6">
                <h3 className={`text-lg font-black tracking-tighter pl-2 ${darkMode ? 'text-white' : 'text-emerald-950'}`}>Timeline Calendar</h3>
                <button 
                  onClick={() => setIsCalendarCollapsed(true)} 
                  className={`p-2 transition-all rounded-xl active:scale-95 ${darkMode ? 'bg-zinc-800 text-zinc-400 hover:bg-orange-500 hover:text-white' : 'text-emerald-400 hover:text-emerald-600'}`}
                >
      ...
      <div className="flex-1 space-y-4 w-full h-full flex flex-col overflow-visible">
        <div className={`flex flex-col sm:flex-row sm:items-center justify-between ${darkMode ? 'bg-zinc-900 border-white/10 text-emerald-100 shadow-[0_8px_32px_rgba(0,0,0,0.6)]' : 'bg-white/40 border-white/20 text-emerald-900'} backdrop-blur-3xl p-1.5 sm:p-1 rounded-2xl flex-shrink-0 border gap-2 sticky top-0 z-30 shadow-sm transition-all duration-500`}>
           <div className="flex items-center gap-1 overflow-x-auto scrollbar-none pb-0.5 sm:pb-0">
             <button 
               onClick={() => setIsCalendarCollapsed(!isCalendarCollapsed)} 
               className={`p-2 rounded-xl transition-all shadow-sm flex-shrink-0 border border-transparent active:scale-95 ${darkMode ? 'bg-zinc-800 text-zinc-500 hover:bg-emerald-500 hover:text-white hover:shadow-black' : 'text-emerald-600 hover:bg-white/60'}`}
             >
           ...
           <div className="flex items-center justify-between sm:justify-end gap-3 px-2 sm:px-0">
              {statsView === 'day' && (
                <div className={`p-1 rounded-2xl flex border shadow-sm ${darkMode ? 'bg-zinc-950/50 border-white/5 shadow-inner' : 'bg-white/30 border-white/20'} backdrop-blur-md`}>
                   <button 
                     onClick={() => setDayViewMode('timeline')} 
                     className={`p-2 rounded-xl transition-all active:scale-95 ${dayViewMode === 'timeline' ? (darkMode ? 'bg-emerald-500 text-white shadow-black' : 'bg-emerald-600 text-white shadow-md') : (darkMode ? 'bg-zinc-800 text-zinc-600 hover:bg-emerald-500 hover:text-white' : 'text-emerald-400 hover:text-emerald-600')}`} 
                     title="Timeline View"
                   >
                     <LayoutGrid size={16}/>
                   </button>
                   <button 
                     onClick={() => setDayViewMode('stats')} 
                     className={`ml-1 p-2 rounded-xl transition-all active:scale-95 ${dayViewMode === 'stats' ? (darkMode ? 'bg-emerald-500 text-white shadow-black' : 'bg-emerald-600 text-white shadow-md') : (darkMode ? 'bg-zinc-800 text-zinc-600 hover:bg-emerald-500 hover:text-white' : 'text-emerald-400 hover:text-emerald-600')}`} 
                     title="Stats View"
                   >
                     <BarChart size={16}/>
                   </button>
        ...
        <div className="flex-1 pr-2 space-y-4 scrollbar-none pb-20">
                      ...
                      >
                        {!isZoomToolExpanded ? (
                          <button 
                            ...
                            className={`w-full h-full flex items-center justify-center ${darkMode ? 'bg-zinc-800 text-zinc-500 hover:text-emerald-500 hover:bg-zinc-700' : 'text-emerald-600'} transition-colors`}
                        ...
                        <div className="h-full relative py-14 px-10" style={{ width: `${getTimelineWidth(timelineZoom)}px` }}>
                          ...
                          <div className="relative z-10 space-y-2 mt-2">
                              ...
                              <div key={trackIdx} className="h-6 relative">
                                {track.map(log => (
                                  <div key={log.id} onClick={() => handleViewLog(log)} className="absolute top-0 bottom-0 rounded-xl cursor-pointer transition-all hover:brightness-125 hover:shadow-[0_0_20px_rgba(255,255,255,0.2)] hover:z-50 border border-white/20 shadow-sm group/log z-10 overflow-visible" style={{ left: `${((log.startTime - timelineRange.start) / 60000) * 1.5 * timelineZoom + 40}px`, width: `${Math.max(((log.endTime && log.startTime ? Math.max(log.duration, (log.endTime - log.startTime) / 1000) : log.duration) / 60) * 1.5 * timelineZoom, 6)}px`, backgroundColor: getCategoryColor(log.category) }}>
                                    <div className={`absolute hidden group-hover/log:flex flex-col items-center ${darkMode ? 'bg-black/90 text-emerald-300 border border-emerald-500/30' : 'bg-emerald-900 text-white'} p-3 rounded-[1.2rem] text-[10px] top-full mt-3 left-1/2 -translate-x-1/2 z-[100] shadow-[0_12px_44px_rgba(0,0,0,0.8)] backdrop-blur-3xl whitespace-nowrap animate-in fade-in zoom-in-95 duration-200 ring-1 ring-white/5`}>
                                      <div className="font-black uppercase tracking-widest mb-1">{log.category}</div>
                                      <div className="opacity-80 font-mono font-bold">{formatClock(log.startTime, timelineZoom)} - {log.endTime ? formatClock(log.endTime, timelineZoom) : 'NOW'}</div>
                                    </div>
                                  </div>
                                ))}
                              </div>
                   ...
                   <div className="space-y-6 pt-2">
                     <div className={`flex items-center justify-between border-b ${darkMode ? 'border-white/5' : 'border-white/10'} pb-5`}>
                       <div className="flex items-center gap-4">
                         <div className={`p-3 ${darkMode ? 'bg-zinc-800 shadow-black' : 'bg-emerald-600 shadow-emerald-200/40'} text-white rounded-2xl shadow-lg transition-transform`}>
                           <History size={24} className={darkMode ? 'text-emerald-500' : 'text-white'}/>
                         </div>
                         <div>
                           <h4 className={`text-xl font-black ${darkMode ? 'text-white' : 'text-emerald-950'} tracking-tight`}>Life Timeline</h4>
                           <p className={`text-[10px] font-black ${darkMode ? 'text-zinc-600' : 'text-emerald-400/50'} uppercase tracking-widest mt-1`}>{formatDisplayDateString(selectedStatsDate)}</p>
                         </div>
                       </div>
                       <button 
                         onClick={() => setActiveTab('logs')} 
                         className={`px-5 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all flex items-center gap-2 border active:scale-95 ${darkMode ? 'bg-zinc-800 text-zinc-400 border-white/5 hover:bg-emerald-500 hover:text-white shadow-black' : 'bg-white/60 text-emerald-600 border-white hover:bg-emerald-600 hover:text-white shadow-sm'}`}
                       >
                     ...
                     <div className="space-y-4 flex-1 min-w-0">
                       {selectedDayLogs.map((log, idx) => (
                         <div key={log.id} className="relative animate-in slide-in-from-left duration-700 ml-10" style={{ animationDelay: `${idx * 80}ms` }}>
                           <div className="absolute -left-[30px] top-6 w-4 h-4 rounded-full flex items-center justify-center ring-4 ring-white/50 z-10 shadow-[0_0_12px_rgba(0,0,0,0.2)]" style={{ backgroundColor: getCategoryColor(log.category) }}>
                              <div className="w-1.5 h-1.5 bg-white rounded-full"/>
                           </div>
                           
                           {/* Connecting Line */}
                           {idx < selectedDayLogs.length - 1 && (
                             <div className={`absolute -left-[22.5px] top-10 bottom-[-24px] w-0.5 ${darkMode ? 'bg-white/10' : 'bg-white/30'}`} />
                           )}
                           ...
                           <div
                             ...
                             className={`${darkMode ? 'bg-zinc-900 border-white/10 hover:bg-zinc-800 hover:border-emerald-500/50 shadow-[0_8px_32px_rgba(0,0,0,0.8)]' : 'bg-white/70 border-white/60 shadow-sm hover:shadow-lg'} backdrop-blur-3xl p-3.5 rounded-2xl border hover:-translate-y-1 transition-all duration-300 focus:outline-none focus-visible:ring-2 focus-visible:ring-emerald-100 cursor-pointer group flex gap-5 relative overflow-hidden`} 
                           >
                              {/* Left Side Accent */}
                              <div 
                                ...
                                <div className="flex items-start justify-between gap-4 mb-2">
                                  <div className="space-y-1">
                                    <div className="flex items-center gap-2">
                                      <div 
                                        className="p-1 rounded-md transition-colors group-hover:bg-emerald-500/40"
                                      ...
                                      <span className={`text-[9px] font-black ${darkMode ? 'text-emerald-400/60 group-hover:text-emerald-300' : 'text-emerald-900/40'} uppercase tracking-widest`}>{log.category}</span>
                                    </div>
                                    <h4 className={`text-[14px] font-black ${darkMode ? 'text-emerald-100 group-hover:text-white' : 'text-emerald-950 group-hover:text-emerald-700'} tracking-tight leading-tight transition-colors line-clamp-1`}>
                                      {log.description || 'Focus Session'}
                                    </h4>
                                  </div>
                                  <div className={`${darkMode ? 'bg-emerald-500/30 text-emerald-100 border-emerald-500/30 group-hover:bg-emerald-500 group-hover:text-white transition-all duration-300' : 'bg-white/60 text-emerald-600 border-white/20'} backdrop-blur-md px-2 py-1 rounded-lg text-[11px] font-black tracking-tighter tabular-nums border shadow-sm`}>
                                  ...
                                  <div className="flex items-center gap-2" onClick={(e) => e.stopPropagation()}>
                                    {log.images.length > 0 && (
                                      <div className="flex -space-x-1.5">
                                        {log.images.slice(0, 3).map((img, imgIdx) => (
                                          <div 
                                            key={imgIdx} 
                                            className={`w-7 h-7 rounded-lg border-2 ${darkMode ? 'border-white/10' : 'border-white'} shadow-sm overflow-hidden hover:scale-110 hover:z-10 transition-all cursor-zoom-in`}
                                    ...
                                    {log.link && (
                                      <a 
                                        ...
                                        className={`w-7 h-7 ${darkMode ? 'bg-white/5 text-emerald-400 border-white/10 hover:bg-emerald-500' : 'bg-emerald-50 text-emerald-500 hover:bg-emerald-600 border-emerald-100/50'} hover:text-white rounded-lg flex items-center justify-center transition-all border group/link`}
                                        title={log.link}
                                      >
                 ...
                 <div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-700">
                   <div className="grid grid-cols-1 lg:grid-cols-12 gap-3">
                      <div className="lg:col-span-3 space-y-2">
                         <div className={`${darkMode ? 'bg-zinc-800' : 'bg-emerald-600'} text-white px-4 py-3 rounded-[1.5rem] shadow-xl ${darkMode ? 'shadow-black/80 border border-white/10' : 'shadow-emerald-100/50'} relative overflow-hidden group min-h-[64px] flex flex-col justify-center`}>
                         ...
                         <div className={`${darkMode ? 'bg-zinc-900 border-white/10' : 'bg-white/40 border-white/20'} backdrop-blur-xl border px-4 py-3 rounded-[1.5rem] shadow-sm relative overflow-hidden group min-h-[64px] flex flex-col justify-center`}>
                           <span className={`text-[11px] font-bold tracking-tight ${darkMode ? 'text-white/60' : 'text-emerald-600'} relative z-10`}>Rest</span>
                           <div className={`text-xl font-black tracking-tighter relative z-10 font-mono ${darkMode ? 'text-white' : 'text-emerald-900'}`}>
                      ...
                      <div className="lg:col-span-9 grid grid-cols-2 sm:grid-cols-3 xl:grid-cols-5 gap-2 pb-3">
                        {statsData.filter(item => item.name !== 'Rest').map((item, idx) => (
                          <div 
                            key={item.name} 
                            className={`backdrop-blur-xl px-3 py-2 rounded-xl border transition-all flex flex-col justify-center group/card ${
                              darkMode 
                                ? 'bg-white/5 border-white/5 hover:bg-white/10 hover:-translate-y-1' 
                                : 'bg-white/40 border-white/20 shadow-sm hover:shadow-md hover:-translate-y-0.5'
                            }`} 
                            style={{ animationDelay: `${idx * 20}ms` }}
                          >
                            <div className="flex items-center gap-2 mb-1.5">
                              <div 
                                className="w-9 h-9 rounded-lg flex items-center justify-center text-sm shadow-sm flex-shrink-0 transition-transform group-hover/card:scale-110" 
                                style={{ 
                                  backgroundColor: darkMode ? `${getCategoryColor(item.name as Category)}30` : `${getCategoryColor(item.name as Category)}15`, 
                                  color: getCategoryColor(item.name as Category) 
                                }}
                              >
                                {React.createElement(getCategoryIcon(item.name as Category), { size: 16 })}
                              </div>
                              <div className="flex flex-col min-w-0">
                                <span className={`text-[11px] font-bold ${darkMode ? 'text-emerald-400/40' : 'text-emerald-900/40'} tracking-tight leading-none mb-1 truncate`}>{item.name}</span>
                                <div className={`text-sm font-black ${darkMode ? 'text-emerald-100' : 'text-emerald-950'} truncate leading-none`}>{item.value}m</div>
                              </div>
                            </div>
                            <div className={`w-full h-1.5 ${darkMode ? 'bg-black/40' : 'bg-white/20'} rounded-full overflow-hidden`}>
                   ...
                   <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 animate-in zoom-in-95 duration-500">
                      <div className={`${darkMode ? 'bg-zinc-900/40 border-white/10 shadow-[0_8px_32px_rgba(0,0,0,0.5)]' : 'bg-white/40 border-white/20 shadow-sm'} backdrop-blur-xl p-6 rounded-[2.5rem] border h-[380px] flex flex-col relative overflow-hidden group`}>
                        <h3 className={`text-sm font-black mb-4 ${darkMode ? 'text-white' : 'text-emerald-800'} tracking-tight relative z-10 flex items-center gap-2`}><div className={`w-1.5 h-4 ${darkMode ? 'bg-emerald-400' : 'bg-emerald-600'} rounded-full`}/> Category Breakdown</h3>
                        {statsData.length > 0 ? (
                      ...
                      <div className={`${darkMode ? 'bg-zinc-900/40 border-white/10 shadow-[0_8px_32px_rgba(0,0,0,0.5)]' : 'bg-white/40 border-white/20 shadow-sm'} backdrop-blur-xl p-6 rounded-[2.5rem] border h-[380px] flex flex-col relative overflow-hidden group`}>
                        <h3 className={`text-sm font-black mb-4 ${darkMode ? 'text-white' : 'text-emerald-800'} tracking-tight relative z-10 flex items-center gap-2`}><div className={`w-1.5 h-4 ${darkMode ? 'bg-emerald-400' : 'bg-emerald-600'} rounded-full`}/> Time Distribution</h3>
                        <ResponsiveContainer width="100%" height="100%">
          ...
          {statsView === 'year' && (
            <div className={`space-y-4 pb-10 animate-in fade-in slide-in-from-bottom-4 w-full ${darkMode ? 'text-zinc-100' : 'text-emerald-900'}`}>
              <div className={`${darkMode ? 'bg-zinc-950/40 border-white/5 ring-white/5 shadow-inner' : 'bg-white border-emerald-50 ring-emerald-50/50 shadow-sm'} rounded-[2.5rem] p-5 border overflow-hidden ring-1`}>
                        ...
                        <div className="space-y-1 px-0.5">
                          {m.categories.filter(cat => cat.name !== 'Rest').slice(0,2).map(cat => (
                            <div key={cat.name} className="flex items-center gap-1.5">
                              ...
                              <div className={`text-[9px] font-bold ${darkMode ? 'text-emerald-100/40' : 'text-emerald-800/60'} truncate tracking-tight leading-none`}>{cat.name}</div>
                              <div className={`ml-auto text-[9px] font-bold ${darkMode ? 'text-emerald-400' : 'text-emerald-400'} leading-none`}>{cat.minutes}m</div>
                            </div>
                          ))}
                        </div>
                      </div>
               ...
               <div className="grid grid-cols-1 lg:grid-cols-12 gap-3 animate-in slide-in-from-bottom duration-500 delay-150">
                  <div className="lg:col-span-3 space-y-2">
                     <div className={`${darkMode ? 'bg-zinc-800' : 'bg-emerald-600'} text-white px-4 py-3 rounded-[1.25rem] shadow-xl ${darkMode ? 'shadow-black/70 border border-white/10' : 'shadow-emerald-100/50'} relative overflow-hidden group min-h-[64px] flex flex-col justify-center`}>
                     ...
                     <div className={`${darkMode ? 'bg-zinc-900 border-white/10' : 'bg-white border-emerald-50 shadow-sm'} px-4 py-3 rounded-[1.25rem] relative overflow-hidden border group min-h-[64px] flex flex-col justify-center`}>
                       <span className={`text-[10px] font-bold tracking-tight ${darkMode ? 'text-white/40' : 'text-emerald-300'} relative z-10`}>Rest</span>
                       <div className={`text-xl font-bold mt-0.5 tracking-tighter relative z-10 font-mono ${darkMode ? 'text-white' : 'text-emerald-900'}`}>
                         {formatTime(restTimeTotal)}
                       </div>
                     </div>
                     <div className={`px-5 py-0.5 text-[10px] font-bold ${darkMode ? 'text-emerald-300/40' : 'text-emerald-300'} tracking-tight`}>{relevantLogs.length} sessions year</div>
                  </div>
                  
                  <div className="lg:col-span-9 grid grid-cols-2 sm:grid-cols-3 xl:grid-cols-5 gap-2 pb-1">
                    {statsData.filter(item => item.name !== 'Rest').map((item, idx) => (
                      <div key={item.name} className={`${darkMode ? 'bg-white/5 border-white/5 hover:bg-white/10' : 'bg-white border-emerald-50 shadow-sm hover:shadow-md'} px-3 py-2.5 rounded-[1.2rem] border transition-all hover:-translate-y-0.5 flex flex-col justify-center ring-1 ring-white/5`} style={{ animationDelay: `${idx * 20}ms` }}>
                        <div className="flex items-center gap-2 mb-2">
                          <div className="w-9 h-9 rounded-full flex items-center justify-center text-xs shadow-sm flex-shrink-0" style={{ backgroundColor: darkMode ? `${getCategoryColor(item.name as Category)}20` : `${getCategoryColor(item.name as Category)}15`, color: getCategoryColor(item.name as Category) }}>
                            {React.createElement(getCategoryIcon(item.name as Category), { size: 16 })}
                          </div>
                          <div className="flex flex-col min-w-0">
                            <span className={`text-[11px] font-bold ${darkMode ? 'text-emerald-100/40' : 'text-emerald-900/40'} tracking-tight leading-none mb-1 truncate`}>{item.name}</span>
                            <div className={`text-sm font-bold ${darkMode ? 'text-emerald-100' : 'text-emerald-950'} truncate leading-none tracking-tight`}>{item.value}m</div>
                          </div>
                        </div>
                        <div className={`w-full h-1.5 ${darkMode ? 'bg-black/40' : 'bg-emerald-50/70'} rounded-full overflow-hidden`}>
                           <div className="h-full rounded-full transition-all duration-1000" style={{ width: `${(item.value / Math.max(1, statsData.reduce((a,b)=>a+b.value,0)))*100}%`, backgroundColor: getCategoryColor(item.name as Category) }} />
                        </div>
                      </div>
                    ))}
