...
const LoggingModal: React.FC<LoggingModalProps> = ({
  ...
  return (
    <div className={`fixed inset-0 ${darkMode ? 'bg-zinc-950/80 backdrop-blur-md' : (wasMiniModeBeforeModal || isMiniMode) ? 'bg-transparent' : 'bg-emerald-900/60 backdrop-blur-xl'} flex items-center justify-center p-6 z-[170] animate-in fade-in duration-300`}>
      ...
      `}</style>
      <div className={`${darkMode ? 'bg-zinc-900 border-none shadow-[0_32px_128px_-20px_rgba(0,0,0,0.9)]' : 'bg-white ring-1 ring-emerald-100/50 shadow-2xl'} rounded-[3rem] p-7 max-w-lg w-full relative max-h-[90vh] overflow-hidden flex flex-col`} style={{ WebkitAppRegion: 'drag' } as any}>
         <div style={{ WebkitAppRegion: 'no-drag' } as any} className="flex flex-col min-h-0 flex-1">
           ...
           <h2 className={`text-2xl font-black mb-6 tracking-tight flex-shrink-0 ${darkMode ? 'text-white' : 'text-emerald-950'}`}>Session Detail</h2>
           <div className="space-y-5 overflow-y-auto pr-2 logging-modal-scroll" style={{ scrollbarWidth: 'thin', scrollbarColor: darkMode ? '#10b981 transparent' : '#10b981 #f0f9f0' } as any}>
